...
final class AppState {
    ...
    private var stateFileMonitors: [StateFileMonitor] = []
    private var agentStates: [String: LampState] = [:]
    ...
    func setup() {
        ...
        // Each agent writes to its own file — no more race conditions
        let agentFiles = [
            ("cc", "/tmp/moonside_cc"),
            ("ag", "/tmp/moonside_ag"),
            ("cx", "/tmp/moonside_cx"),
        ...
        for (agent, path) in agentFiles {
            let monitor = StateFileMonitor(path: path)
            ...
            monitor.start()
            stateFileMonitors.append(monitor)
        }

        checkIntegrations()
    }

    /// Resolves the highest-priority state across all agents.
    /// input_* > working_* > idle > off
    private func resolveAgentPriority() -> LampState {
        let states = Array(agentStates.values)
        let inputStates: Set<LampState> = [.inputCC, .inputAG, .inputCX]
        let workingStates: Set<LampState> = [.working, .workingAG, .workingCX]
        // Any agent needs attention → show that
        if let input = states.first(where: { inputStates.contains($0) }) { return input }
        // Any agent working → show that
        if let working = states.first(where: { workingStates.contains($0) }) { return working }
        // All idle or off
    ...
    func toggleLed() {
        ...
        if isLedOn {
            ...
            // Write off to all agent files
            for path in ["/tmp/moonside_cc", "/tmp/moonside_ag", "/tmp/moonside_cx"] {
                try? "off".write(toFile: path, atomically: true, encoding: .utf8)
            }
            agentStates = [:]
        }
    }
